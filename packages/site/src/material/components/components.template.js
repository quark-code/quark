...
const config = {
    ...
    include: ['material_demos_', 'material_samples_']
}
...
function getSuperclassDetail(item) {
    if (item.inheritedFrom) {
        return `superclass="${item.inheritedFrom.name}" superclass-url="/material/components/${item.inheritedFrom.name}/"`;
    }
...
const render = (data, include) => {
    ...
    return allComponents.map(component => {
        const demoName = `material_demos_${component.name}`;
        const sampleName = `material_samples_${component.name}`;
        const hasSuperclassUrl = component.superclass && component.superclass.name && component.superclass.name !== 'LitElement';
        const superclassUrl = hasSuperclassUrl ? `/material/components/${component.superclass.name}/` : '';
...
function buildExtends(superClass) {
    if ((superClass && superClass !== 'LitElement')) {
        return html`<a href="/components/${superClass}/">${superClass}</a>`;
    } else {
        return superClass;
    }
}
        ...
        return {
            ...
            content: html`
                <div class="component-content">
                    <doc-component-header component-name="${component.name}" component-display-name="${component.displayName}" 
                                          component-tag="${component.tagName}" component-summary="${component.summary}" 
                                          component-superclass="${component.superclass.name}" component-superclass-package="${component.superclass.package}">
                    </doc-component-header>
                    <doc-component-tabs page-key="${component.name}">
                        ...
                        <div slot="documentation">
                            ${component.attributes && component.attributes.length > 0 ? html`
                                <doc-component-api-section caption="Attributes" page-key="${component.name}">
                                        ${component.attributes.map(item => html`
                                            <div class="component-part">
                                                <span class="component-part-name">${escape(item.name)}</span>
                                                ${item.type && item.type.text ? html`<span class="component-part-name">: </span><span class="component-part-values">${item.type.text}</span>` : null}
                                                ${item.inheritedFrom ? html`<div class="component-part-inherited">Inherited from ${buildExtends(item.inheritedFrom.name)}</div>` : null}
                                                ${item.description ? html`<div class="component-part-description">${escape(item.description)}</div>` : null}
                                            </div>
                                        `)}
                                </doc-component-api-section>
                            `: null}

                            ${component.properties && component.properties.length > 0 ? html`
                                <doc-component-api-section caption="Properties" page-key="${component.name}">
                                    ${component.properties.filter(item => !item.protected).map(item => html`
                                        <div class="component-part">
                                            <span class="component-part-name">${escape(item.name)}:</span>
                                            <span class="component-part-values">${item.type ? escape(item.type.text + ' |') : ''} ${item.default ? escape(item.default) : ''}</span>
                                            ${item.readonly ? html`<div class="readonly-property">readonly</div>` : null}
                                            ${item.inheritedFrom ? html`<div class="component-part-inherited">Inherited from ${buildExtends(item.inheritedFrom.name)}</div>` : null}
                                            ${item.description ? html`<div class="component-part-description">${escape(item.description)}</div>` : null}
                                        </div>
                                    `)}
                                    ...
                                    ${protectedPropertyCount > 0 ? html`
                                        <doc-component-protected-section content-type="property" content-count="${protectedPropertyCount}" page-key="${component.name}">
                                            ${component.properties.filter(item => item.protected).map(item => html`
                                                <div class="component-part">
                                                    <span class="component-part-name">${escape(item.name)}:</span>
                                                    <span class="component-part-values">${item.type ? escape(item.type.text + ' |') : ''} ${item.default ? escape(item.default) : ''}</span>
                                            ...
                                            `)}
                                        </doc-component-protected-section>
                                    ` : null }
                                </doc-component-api-section>
                            `: null}
                            
                            ${component.methods && component.methods.length > 0 ? html`
                                <doc-component-api-section caption="Methods" page-key="${component.name}">
                                    ${component.methods.filter(item => !item.protected).map(item => html`
                                        <div class="component-part">
                                            <span class="component-part-name">${escape(item.name)}</span>
                                            <span class="component-part-values">${buildMethodParameters(item.parameters)}: ${buildMethodReturn(item.return)}</span>
                                            ${item.inheritedFrom ? html`<div class="component-part-inherited">Inherited from ${buildExtends(item.inheritedFrom.name)}</div>` : null}
                                            ${item.description ? html`<div class="component-part-description">${escape(item.description)}</div>` : null}
                                        </div>
                                    `)}
                                    ...
                                    ${protectedMethodCount > 0 ? html`
                                        <doc-component-protected-section content-type="method" content-count="${protectedMethodCount}" page-key="${component.name}">
                                            ${component.methods.filter(item => item.protected).map(item => html`
                                                <div class="component-part">
                                                    <span class="component-part-name">${escape(item.name)}</span>
                                                    <span class="component-part-values">${buildMethodParameters(item.parameters)}: ${buildMethodReturn(item.return)}</span>
                            ...
                            ${component.events && component.events.length > 0 ? html`
                                <doc-component-api-section caption="Events" page-key="${component.name}">
                                    ${component.events.map(item => html`
                                        <div class="component-part">
                                            <span class="component-part-name">${escape(item.name)}${item.type ? ':' : ''}</span>
                                            ${item.type ? html`<span class="component-part-values">${escape(item.type.text)}</span>` : null}
                                            ${item.inheritedFrom ? html`<div class="component-part-inherited">Inherited from ${buildExtends(item.inheritedFrom.name)}</div>` : null}
                                            ${item.description ? html`<div class="component-part-description">${escape(item.description)}</div>` : null}
                                        </div>
                                    `)}
                            ...
                            ${component.slots && component.slots.length > 0 ? html`
                                <doc-component-api-section caption="Slots" page-key="${component.name}">
                                    ${component.slots.map(item => html`
                                        <div class="component-part">
                                            <span class="component-part-name">${item.name ? escape(item.name) : '(default)'}</span>      
                                            ${item.description ? html`<div class="component-part-description">${escape(item.description)}</div>` : null}
                                        </div>
                                    `)}
                                </doc-component-api-section>
                            ...
                            ${component.cssProperties && component.cssProperties.length > 0 ? html`
                                <doc-component-api-section caption="CSS Properties" page-key="${component.name}">
                                    ${component.cssProperties.map(item => html`
                                        <div class="component-part">
                                            <span class="component-part-name">${escape(item.name)}:</span>
                                            <span class="component-part-values">${buildCssPropertyValue(item.default)}</span>
                                            ${item.description ? html`<div class="component-part-description">${escape(item.description)}</div>` : null}
                                        </div>
                                    `)}
                                </doc-component-api-section>
                            ...
                            ${component.cssParts && component.cssParts.length > 0 ? html`
                                <doc-component-api-section caption="CSS Parts" page-key="${component.name}">
                                    ${component.cssParts.map(item => html`
                                        <div class="component-part">
                                            <span class="component-part-name">${escape(item.name)}</span>
                                            ${item.description ? html`<div class="component-part-description">${escape(item.description)}</div>` : null}
