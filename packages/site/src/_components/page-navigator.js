...
export class PageNavigator extends QuarkElement {
    ...
    _renderItemLabel(item, key) {
        return (item.items && item.items.length > 0) ? html`
            <button role="button" type="button" aria-disabled="false" class="item-label" key="${key}" @click="${this._itemToggle}" root>${item.label} ${this._renderIcon(item)}</button>
        ` : html`
    ...
    _selectItem(el) {
        [...this.renderRoot.querySelectorAll('.item-label-link[selected]')].forEach(el => el.removeAttribute('selected'));
    ...
    _setState(collapsed = true, selected = true, filter = true) {
        ...
        if (collapsed) {
            this._collapsedItems = [...this.renderRoot.querySelectorAll('.item-label[collapsed')].map(item => item.getAttribute('key'));
    ...
    _updateToggledItems() {
        ...
        if ((this._collapsedItems) && (this._collapsedItems.length > 0)) {
            const items = [...this.renderRoot.querySelectorAll('.item-label[key]')];
