...
export class SearchField extends QuarkElement {
    ...
    @property({ type: Boolean, attribute: 'has-focus', reflect: true })
    ...
    _handleInputFocusEvent(e) {
        this.hasFocus = true;
        (this.renderRoot as ShadowRoot).getElementById('input').focus();
    }

    _handleInputBlurEvent(e) {
        this.hasFocus = false;
        (this.renderRoot as ShadowRoot).getElementById('input').blur();
    }
